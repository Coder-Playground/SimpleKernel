...
int32_t SLAB::init(void) {
    ...
    heap_free   = PMM_PAGE_SIZE - sizeof(list_entry_t);
    block_count = 1;
    slab_list   = sb_list;
    list_init(slab_list);
    // 设置第一块内存的相关信息
    list_slab_block(slab_list)->allocated = SLAB_UNUSED;
    list_slab_block(slab_list)->len = PMM_PAGE_SIZE - sizeof(list_entry_t);
    io.printf("slab init.\n");
    return;
}
...
// 返回分割出来的管理头地址
list_entry_t *SLAB::slab_split(list_entry_t *entry, size_t len) {
...
// 合并内存块
void SLAB::slab_merge(list_entry_t *list) {
...
// 寻找长度符合的项
list_entry_t *SLAB::find_entry(size_t len) {
    list_entry_t *entry = slab_list;
    do {
...
void *SLAB::alloc(size_t byte) {
    ...
    size_t        pages     = (len % PMM_PAGE_SIZE == 0) ? (len / PMM_PAGE_SIZE)
                                                         : ((len / PMM_PAGE_SIZE) + 1);
    list_entry_t *new_entry = (list_entry_t *)pmm.alloc_page(pages);
    if (new_entry == NULL) {
